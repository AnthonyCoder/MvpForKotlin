...
class HomeFragment : BaseFragment<HomePresenter>(), HomeContact.View {
    ...
    override fun onBanner(bannerResults: List<BannerResult.DataBean>) {
            if (bannerResults.isNotEmpty()) {
    ...
    override fun onWeChatAuthors(weChatAuthorResults: List<WeChatAuthorResult.DataBean>) {
        ...
        gridViewPager.setAdapter(object : GridViewPagerAdapter<WeChatAuthorResult.DataBean>(weChatAuthorResults) {
            override fun bindData(
                ...
                t?.let {
                    ...
                    viewHolder.setText(R.id.tv_home_author_icon, it.name?.get(0)?.toString())
    ...
    private fun getImages(bannerResults: List<BannerResult.DataBean>): List<String> {
        ...
        if (bannerResults != null) {
            for (bannerResult in bannerResults) {
                bannerResult.imagePath?.let { list.add(it) }
            }
        }
        return list
    }
    ...
    private fun gotoWebViewActivityFromBanner(bannerResult: BannerResult.DataBean?) {
        ...
        ARouter.getInstance().build(ARouterConstants.COMMON_X5WEB_ACTIVITY).withString("url", bannerResult.url)
            .withString("webTitle", bannerResult.title).navigation()
    ...
    private fun gotoWeChatArticleListActivity(weChatAuthorResult: WeChatAuthorResult.DataBean) {
        ARouter.getInstance().build(ARouterConstants.HOME_WECHAT_ACTICLELIST_ACTIVITY)
            .withInt("id", weChatAuthorResult.id).withString("title", weChatAuthorResult.name).navigation()
    }

    private fun gotoWebViewActivity(datasBean: HomeArticleResult.DataBean.DatasBean) {
        ARouter.getInstance().build(ARouterConstants.COMMON_X5WEB_ACTIVITY).withString("url", datasBean.link)
            .withString("webTitle", datasBean.title).navigation()
    }
