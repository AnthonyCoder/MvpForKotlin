...
class ProjectFragment : BaseFragment<ProjectPresenter>(), ProjectContact.View {
    ...
    override fun onProjectTabs(itemList: List<ProjectTreeResult.DataBean>) {
    ...
    private fun createProjectPages(projectItems: List<ProjectTreeResult.DataBean>?): List<ProjectPageItem> {
        ...
        for (projectItem in projectItems) {
            val projectPageItem = ProjectPageItem(
                projectItem.id,
                projectItem.name,
                ARouter.getInstance().build(ARouterConstants.PROJECT_PAGE_FRAGMENT).withInt(
                    "pageId",
                    projectItem.id
