...
class CategoriesFragment : BaseFragment<CategoriesPresenter>(), CategoriesContact.View {
    ...
    override fun setCategories(list: List<GetCategoriesResult>) {
        dataList.clear()
        ...
        categoriesAdapter!!.onItemClickListener = BaseQuickAdapter.OnItemClickListener { adapter, view, position ->
            ARouter.getInstance().build(ARouterConstants.KAIYAN_VIDEOLIST_ACTIVITY).withInt("id", dataList[position].id)
                .navigation()
        }
