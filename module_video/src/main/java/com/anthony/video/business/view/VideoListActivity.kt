...
class VideoListActivity: BaseActivity<VideoListPresenter>(),VideoListContact.View {
    ...
    override fun setVideoList(dataList: List<VideoListResult.ItemListBean>) {
        ...
        videAdapter!!.onItemClickListener = BaseQuickAdapter.OnItemClickListener { adapter, view, position ->
            dataList?.get(position)?.let {
                ARouter.getInstance().build(ARouterConstants.COMMON_X5WEB_ACTIVITY).withString("url",
                    it.data?.webUrl?.raw
                ).withString("webTitle", it.data?.title).navigation()
            }
